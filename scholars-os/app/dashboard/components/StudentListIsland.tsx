...
export function StudentListIsland({
  ...
  const [previewStudents, setPreviewStudents] = useState<StudentRow[]>(initialStudents)
  const [previewFilteredCount, setPreviewFilteredCount] = useState(initialFilteredCount)
  const skipFirstFetch = useRef(true)

  useEffect(() => {
    startTransition(() => {
      setPreviewStudents(initialStudents)
      setPreviewFilteredCount(initialFilteredCount)
    })
  }, [initialStudents, initialFilteredCount])
      ...
      fetch(`/api/dashboard/students-preview?${params}`)
        .then(res => {
          ...
          return res.json() as Promise<{
            data: { filteredCount: number; students: StudentRow[] }
          }>
        })
        .then(json => {
          startTransition(() => {
  ...
  return (
    <section className="os-card os-card-interactive min-w-0 overflow-x-hidden">
      ...
      {previewStudents.length > 0 && previewFilteredCount > 25 ? (
