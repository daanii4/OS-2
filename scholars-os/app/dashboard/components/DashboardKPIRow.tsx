...
export async function DashboardKPIRow({
  ...
  const [
    ...
    prisma.behavioralIncident.count({
      where: { ...scopeFilter, incident_date: { gte: periodStart, lte: now } },
    }),
    prisma.behavioralIncident.count({
      where: { ...scopeFilter, incident_date: { gte: previousPeriodStart, lte: previousPeriodEnd } },
    }),
    prisma.session.count({
      where: { ...scopeFilter, session_date: { gte: periodStart, lte: now } },
    }),
    prisma.session.count({
      where: { ...scopeFilter, session_date: { gte: periodStart, lte: now }, attendance_status: 'no_show' },
    }),
    prisma.session.findMany({
      where: { ...scopeFilter, session_date: { gte: periodStart, lte: now }, goal_completion_rate: { not: null } },
  ...
  const avgGoalCompletion =
    recentGoalRates.length > 0
      ? recentGoalRates.reduce((sum, r) => sum + (r.goal_completion_rate ?? 0), 0) /
  ...
  return (
    <section className="os-kpi-grid">
      ...
      >
        ...
        <p className="os-caption mt-1">
          <span className="text-[var(--color-success)]">
            ↑{' '}
            {avgGoalCompletion !== null ? Math.max(1, Math.round(avgGoalCompletion / 10)) : 0}pts
          </span>{' '}
          last 4 sessions
        </p>
      </div>
      ...
      >
        ...
        <p className="os-caption mt-1">
          <span className="text-[var(--color-regression)]">{noShowsCurrent} no-shows</span> need
          follow-up
        </p>
      </div>
    </section>
