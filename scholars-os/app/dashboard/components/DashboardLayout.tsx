...
function splitProfileName(name: string): { first: string } {
...
function NavLinks({
  ...
  const linkInner = (link: NavLinkItem, active: boolean, index: number) => {
    ...
    return (
      <>
        <Icon size={15} aria-hidden className={cn('shrink-0', active ? 'opacity-100' : 'opacity-60')} />
        <span
          className={cn(
...
export function DashboardLayout({
  ...
  const ownerWelcomeGreeting = hour < 12 ? 'Good morning' : hour < 17 ? 'Good afternoon' : 'Good evening'
    ...
    <div className="min-h-screen min-w-0 overflow-x-hidden bg-[var(--surface-page)]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="os-motif flex h-7 w-7 flex-shrink-0 items-center justify-center overflow-hidden bg-[var(--gold-500)]">
                <img src="/logo-mark.png" alt="Operation Scholars" className="h-[22px] w-[22px] object-contain" />
              </div>
              <span className="os-subhead text-[var(--text-primary)]">Operation Scholars</span>
            </div>
          </div>
          <Link href="/dashboard/students" className="os-btn-primary text-sm">+ Log Session</Link>
        </div>
      )}
      ...
      {!isDesktop && mobileMenuOpen && (
        <div className="fixed inset-0 z-50" aria-modal="true">
          ...
          <aside className="absolute left-0 top-0 flex h-full w-[260px] flex-col bg-[var(--olive-800)] transition-transform duration-100" style={{ transform: 'translateX(0)' }}>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="os-motif flex h-8 w-8 flex-shrink-0 items-center justify-center overflow-hidden bg-[var(--gold-500)]">
                    <img src="/logo-mark.png" alt="Operation Scholars" className="h-6 w-6 object-contain" />
                  </div>
                  <div>
                    <p className="text-[15px] font-normal leading-tight tracking-[-0.01em] text-white" style={{ fontFamily: 'var(--font-dm-serif), Georgia, serif' }}>Operation Scholars</p>
                    <p className="mt-0.5 text-[9px] font-semibold uppercase tracking-[0.08em]" style={{ color: 'rgba(255,255,255,0.35)' }}>Behavioral Intelligence</p>
                  </div>
                </div>
                <button type="button" className="os-btn-icon" style={{ color: 'rgba(255,255,255,0.45)' }} onClick={() => setMobileMenuOpen(false)} aria-label="Close navigation">
            ...
            <nav className="flex-1 overflow-y-auto overflow-x-visible px-3 py-2">
              <NavLinks open onLinkClick={closeMobileMenu} showOrgNav={showOrgNav} tooltipProvider={false} />
            </nav>
            <div className="border-t border-white/[0.06] px-2 py-3">
              <SidebarUserMenu profileName={profileName} profileEmail={profileEmail} profileRole={profileRole} showOrgSettings={showOrgNav} expanded />
            </div>
          </aside>
        </div>
      )}
      ...
      {isDesktop && (
        <>
          <aside className={cn('fixed left-0 top-0 z-40 flex h-full flex-col border-r border-white/[0.06] bg-[#2D3820]', 'transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)]', sidebarOpen ? 'w-60' : 'w-14')}>
            <div className={cn('flex h-16 shrink-0 items-center border-b border-white/[0.08] px-4', sidebarOpen ? 'justify-start gap-3' : 'justify-center px-2')}>
              <div className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-[#D6A033]" style={{ minWidth: 36, minHeight: 36 }}>
                <img src="/logo-mark.png" alt="" className="h-7 w-7 object-contain" />
              </div>
              <div className={cn('min-w-0 overflow-hidden transition-all duration-200 ease-out', sidebarOpen ? 'max-w-[180px] translate-x-0 opacity-100' : 'max-w-0 -translate-x-2 opacity-0')}>
                <p className="text-[14px] font-normal leading-tight text-white" style={{ fontFamily: 'var(--font-dm-serif), Georgia, serif' }}>Operation Scholars</p>
                <p className="mt-0.5 text-[8px] uppercase tracking-[0.1em]" style={{ color: 'rgba(255,255,255,0.4)' }}>Behavioral Intelligence</p>
              </div>
            </div>
            ...
            <div className="flex shrink-0 flex-col gap-1 border-t border-white/[0.06] px-2 py-3">
              <button
                ...
                className={cn('flex h-9 w-full items-center gap-3 rounded-lg px-2.5 text-white/40 transition-all duration-150', 'hover:bg-white/[0.06] hover:text-white/70', 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D6A033]', !sidebarOpen && 'mx-auto w-10 justify-center px-0')}
              >
                <PanelLeftClose size={15} className={cn('shrink-0 transition-transform duration-300', !sidebarOpen && 'rotate-180')} aria-hidden />
                <span className={cn('font-sans text-[12px] font-medium whitespace-nowrap transition-all duration-200', sidebarOpen ? 'opacity-100' : 'pointer-events-none w-0 overflow-hidden opacity-0')}>Collapse</span>
              </button>
              <SidebarUserMenu profileName={profileName} profileEmail={profileEmail} profileRole={profileRole} showOrgSettings={showOrgNav} expanded={sidebarOpen} />
            </div>
          </aside>

          <main className={cn('flex h-[100dvh] min-h-0 min-w-0 flex-col overflow-x-hidden overflow-y-auto overscroll-y-contain', 'w-full transition-[padding-left] duration-300 ease-[cubic-bezier(0.4,0,0.2,1)]', sidebarOpen ? 'pl-60' : 'pl-14')}>
            {/* Escalation banner */}
            {escalatedStudentName && !escalationAcknowledged && (
              <div className="flex w-full min-w-0 shrink-0 flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6" style={{ background: '#DC2626', borderRadius: 0 }}>
                ...
                <button className="flex-shrink-0 rounded px-4 py-2 text-[12px] font-semibold transition-colors sm:text-[13px]" style={{ background: '#FFFFFF', color: '#DC2626', minHeight: 36 }} onClick={() => setShowEscalationModal(true)}>
              ...
              <div className="flex min-w-0 flex-wrap items-center justify-between gap-3">
                <div className="flex min-w-0 items-center gap-3">
                  <div className="min-w-0">
                    ...
                    {(profileRole === 'owner' || profileRole === 'assistant') && showOwnerDayGreeting && (
                      <p className="mt-0.5 text-[12px] text-[var(--text-tertiary)]">
                        {ownerWelcomeGreeting}, {splitProfileName(profileName).first}.{' '}
                ...
                <div className="flex min-w-0 flex-shrink-0 flex-wrap items-center justify-end gap-2">
                  ...
                  <Link href="/dashboard/students" className="os-btn-primary">+ Log Session</Link>
                </div>
              </div>
            </div>
              ...
              {caseloadExport && (
                <div className="os-card-interactive relative overflow-hidden rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="mb-4 flex items-center justify-between">
                    <div>
                      <h3 className="font-sans text-sm font-semibold text-slate-900">Export Caseload Report</h3>
                      <p className="mt-0.5 font-sans text-xs text-slate-500">Generate a PDF caseload report for district meetings</p>
                    </div>
                  </div>
                  {caseloadExport}
                </div>
      ...
      {!isDesktop && (
        <div className="min-w-0 overflow-x-hidden">
          {escalatedStudentName && !escalationAcknowledged && (
            <div className="flex w-full min-w-0 flex-wrap items-center justify-between gap-3 px-4 py-3" style={{ background: '#DC2626', borderRadius: 0 }}>
              ...
              <button className="flex-shrink-0 rounded px-3 py-1.5 text-[12px] font-semibold" style={{ background: '#FFFFFF', color: '#DC2626', minHeight: 32 }} onClick={() => setShowEscalationModal(true)}>
                Acknowledge & Take Action
              </button>
            </div>
          )}
          <div className="space-y-3 px-4 py-4">
            {caseloadExport && (
              <div className="os-card-interactive relative overflow-hidden rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                <h3 className="mb-2 font-sans text-sm font-semibold text-slate-900">Export Caseload Report</h3>
                {caseloadExport}
              </div>
            )}
            {children}
      ...
      {escalatedStudentId && escalatedStudentName ? (
        <EscalationReviewModal
          ...
          onEscalationResolved={() => setEscalationAcknowledged(true)}
        />
      ) : null}
    </div>
  )
