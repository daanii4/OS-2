...
const DEFAULT_LIGHT = 'var(--gold-400)'
const DEFAULT_BG = 'var(--olive-600)'
...
export function StarButton({
  ...
  return (
    <button
      ...
      className={cn(
        'group/star-button relative z-[3] inline-flex h-11 min-h-11 min-w-[min(100%,12rem)] max-w-full items-center justify-center gap-2 overflow-hidden rounded-3xl px-6 py-2 text-sm font-semibold transition-colors disabled:pointer-events-none disabled:opacity-50',
        className,
      )}
      {...props}
    >
      {!reducedMotion ? (
        <div
          className="star-btn-travel absolute inset-0 aspect-square bg-[radial-gradient(ellipse_at_center,var(--light-color),transparent,transparent)]"
          style={
            {
              offsetPath: 'var(--path)',
              offsetDistance: '0%',
      ...
      <div
        className="absolute inset-0 z-[4] overflow-hidden rounded-[inherit] border border-[rgba(92,107,70,0.14)] text-white"
        style={{ borderWidth: 'var(--border-width)' }}
        aria-hidden
      >
        <StarBackground color={backgroundColor} />
      </div>
      <span className="relative z-10 inline-block bg-gradient-to-t from-[var(--olive-900)] via-[var(--olive-700)] to-[var(--olive-500)] bg-clip-text text-transparent">
        {children}
      </span>
    </button>
  )
